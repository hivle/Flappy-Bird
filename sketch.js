console.log('[flappy] build 2026-04-23 anim-v5');
...
class Bird {
    constructor(game) {
        ...
        this.turn = 0;
        this.turnup = false;
        this.sprite = new Sprite(game.birdFrames, 0, 0, 0.25);
        this.dead = false;
        this.start = true;
    }
    ...
    update() {
        ...
        if (this.turnup) {
            this.turn -= Math.PI / 180 * 8;
            if (this.turn < -Math.PI / 180 * 40) this.turnup = false;
        } else {
            this.turn += this.dead ? Math.PI / 40 : Math.PI / 100;
